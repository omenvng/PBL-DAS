...
namespace SuperPostDroidPunk.ViewModels
{
    public class MainWindowViewModel : ViewModelBase
    {
        ...
        public ReactiveCommand<Unit, Unit> AddUploadXss { get; }
        ...
        public MainWindowViewModel(IManagedNotificationManager notificationManager)
        {
            ...
            AddUploadXss = ReactiveCommand.Create(UploadXss);
            _notificationManager = notificationManager;
        }

        private  void UploadXss()
        {
            Thread thread = new Thread((ThreadStart)(() => {
                OpenFileDialog fileDialog = new OpenFileDialog();
                fileDialog.DefaultExt = ".json"; 
                fileDialog.Filter = "JSON docs (.json)|*.json"; 
                ...
                if (fileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    var data = JsonConvert.DeserializeObject<XSSModel>(sr.ReadToEnd());
                    MessageBox.Show(sr.ReadToEnd());
                    int i = 0;
                    foreach (var item in data.XssValues)
                    {
                        Params.Add(new Param() { Key = data.ParamName + "#" + i, IsSelected = true, Value = item });
                    }
